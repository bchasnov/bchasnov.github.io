...
## Iteration 2 — headline results (see `DIARY_ITER2.md`, `UMA_FLAWS.md`, `RETRAIN_SPEC.md`)

Iteration 2 turned iteration 1's UMA-only failure map into a **quantified,
dual-method (UMA + GFN2-xTB), ab-initio-confirmed** one, and answered *when UMA
is worth using given DFT*.

- **The one flaw, confirmed at DFT/CCSD(T):** UMA over-stabilizes an *unquenched,
  localized empty p orbital*. The bare **methyl cation is +104.7 kcal/mol** too
  stable vs B3LYP/def2-TZVP (which matches experiment ±1.6); **bridged vinyl +46
  vs CCSD(T)**; **open β-halonium +55–60 vs DFT**. The error is razor-sharp — one
  alkyl or any π-donor quenches it (ordinary/2°/3°/allylic/benzylic/aromatic and
  **terpene** cations match DFT to **<1 kcal/mol**).
- **Corrections to iteration 1:** the bridgehead "flaw" is **not** one — UMA
  (−3.1), xTB (−3.6) and DFT (−2.9) all agree; the F₂/charge-shift issue is a
  uniform bond contraction, off-domain. Both demoted.
- **For the experimental collaborator's chemistry:** UMA is a reliable terpene-mechanism engine — the
  disease needs a bare empty p, which real tertiary/allylic terpene cations lack.
- **Why UMA given DFT:** speed. `synergy.py` sweeps 20–34 conformers of the
  α-terpinyl / linalyl cations into basins in minutes (DFT sweep ≈ hours), finds
  the *rare* global-min basin a single relaxation misses, and DFT single-points
  confirm the ranking — settling the B1 reference-conformer question. **UMA for
  breadth, DFT for the final number, the failure map for knowing which to trust.**

Code: `uma_core/iter2.py` (dual-method harness), `mechanisms2.py` (H101–H165),
`run_dft.py` (PySCF references), `synergy.py` (UMA-explore + DFT-arbiter).
