...
**RESOLVED — iteration 3, in UMA's favor (H202/H203).** The DFT reference is now
in, at mPW1PW91/6-31G(d). Framed as an isodesmic hydride-transfer stability vs
tert-butyl⁺ (the clean, reference-cancelling comparison; the iter-1 1-vs-2
norbornyl gap conflated the bridgehead penalty with 2-norbornyl's *special*
non-classical stabilization), UMA reproduces the bridgehead destabilization to
**≤2.5 kcal/mol** across a strain series (adamantyl, [2.2.2], 1-norbornyl at −13
kcal). The old iter-2 "not UMA-specific — xTB reproduces it" note is superseded:
run end-to-end, **xTB errs by 23–30 kcal (a geometry failure); UMA does not.**
DFT//UMA == DFT//DFT to 0.01 kcal, so UMA's bridgehead geometry is DFT-quality.
Net: this is **not a UMA flaw** — bridgehead/non-planar cations are a UMA
strength (see `WHERE_UMA_HELPS.md` §5).
...
`WHERE_UMA_HELPS.md` §5): on structure-validated bridgehead cations UMA matches
mPW1PW91/6-31G(d) within **2.5 kcal**; xTB (end-to-end) errs **23–30 kcal**
(a geometry failure). DFT//UMA == DFT//DFT to 0.01 kcal.

## LIMITATION 8 — UMA under-penalizes stretched-bond transition states (H240–H243)
**System.** 1,2-H shift and dyotropic double-1,2-H shift barriers (rigid
interpolation scan; UMA and DFT on identical images).

**Result.** At every stretched (TS-like) geometry UMA sits **below** DFT
(UMA−DFT < 0), maximal at the barrier top and ~0 at the minima. Magnitude scales
with how stretched the TS is: **~1 kcal/mol** for the low barriers of carbocation
1,2-H / dyotropic shifts (2-butyl 2.3 vs 3.1; dyotropic 3.6 vs 4.6), rising to
~5 kcal at a severely-stretched geometry. Same root as the F₂ charge-shift/
stretched-bond weakness (iter-1 FLAW 6), now shown to generalize to TSs.

**Takeaway.** UMA is usable for *qualitative / screening* barriers of the
low-barrier shifts that dominate carbocation rearrangements (within ~1 kcal), but
it systematically under-predicts, and the error grows for high/late TSs — so hand
**quantitative and high barriers to DFT**. A saddle-optimized (NEB/dimer) study
would refine the magnitudes; the directional bias is unambiguous.

## CONFIRMED-then-DEMOTED as a fix path — descriptor Δ-learning cannot repair the cation flaw (H260–H261)
UMA's over-stabilization of bare-empty-p cations (FLAW 5) is **not correctable
from cheap structural descriptors**: the residual is non-smooth in descriptor
space (phenyl vs vinyl, and F- vs O-methyl, are descriptor-degenerate with
opposite errors), so a global Δ-model destroys the applicable systems (whose UMA
error is already ~1 kcal) and a locality-gated one still fails. The regime must be
**detected-and-deferred to DFT** or **retrained with proper labels**, per
`RETRAIN_SPEC.md` — not patched with a residual model.

## OBSERVATION 9 — a possible reference sensitivity on isomerizations (H324/H325, n small — NOT settled)
**Setup.** Two "UMA divergences" flagged against mPW1PW91/6-31G(d) — keto-enol
tautomerization (H313/H314, ~5 kcal) and strained-ring vs alkene/chain
isomerization (H302/H311/H320–H322, ~4 kcal) — were spot-checked with
**CCSD(T)/def2-TZVP** on DFT-optimized geometries.

**Result (4 molecules, one reference each).**

| comparison | UMA − CCSD(T) | DFT/6-31G(d) − CCSD(T) |
|---|---|---|
| acetone enol−keto | −0.96 | +4.08 |
| butanone enol−keto | −1.30 | +3.59 |
| cyclobutane−butene | −0.62 | −4.64 |
| cyclopentane−pentene | −0.19 | −4.06 |

**Tentative read (explicitly not a conclusion).** On these four bonding-type-change
isomerizations (C=O↔C–OH, ring-σ↔alkene-π), UMA happened to sit closer to
CCSD(T)/def2-TZVP than 6-31G(d) did. That is *suggestive* that a UMA–DFT gap on
such an axis need not be assumed a UMA error — but it is **only 4 data points,
single-points on DFT geometries, and def2-TZVP is not the exact/CBS limit**, so it
is not evidence that "UMA beats DFT" or that "the reference is wrong." **Do not
report it as a finding without several more isomerizations at CCSD(T)** (and ideally
a larger basis / CBS check). The one actionable caveat that IS safe: when a
comparison changes bonding type, don't treat a small-basis DFT number as ground
truth — check it against a higher method before ranking UMA against it.

## FLAW 5 — empty-p over-stabilization, now CCSD(T)-CONFIRMED and quantified (H327)
The bare/strained empty-p cation flaw (FLAW 5, iter-1/W7) was re-tested against
the gold standard on the cyclopropyl-cation geometry (C3H5⁺, rel allyl, all levels
on the fixed UMA geometry): **UMA −25.24, DFT +24.62, CCSD(T) +23.06.** DFT and
CCSD(T) agree to 1.6 kcal; **UMA is a −48 kcal outlier** — UMA's PES carries a
spurious deep basin exactly where the true surface is a 23-kcal-uphill,
strained/bare empty-p region. So the flaw is real, large, and CCSD(T)-anchored —
and it is confined to the bare empty-p regime: the moment the empty p-center is
delocalized (allyl, homoallyl, tropylium) or alkyl-substituted, UMA returns to
≤2.6 kcal of both DFT and CCSD(T) (H315–H319, H326 vinyl +2.4). This direction is
on firmer footing than OBSERVATION 9 because **two independent references (DFT and
CCSD(T)) agree** and UMA is the lone outlier — so the gate in `RETRAIN_SPEC.md`
(defer bare empty-p to DFT, trust everything else) has real support, though the
exact magnitude still rests on this one geometry and would benefit from a few more
bare-empty-p cases.
